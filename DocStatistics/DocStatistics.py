...
from DocStatisticsConstants import *

class DSDocStatistics:
	"""
	...
	def lines_in_book(text_book):
		...
		li_lines = [DSDocStatistics.lines_in_bloc(bloc) for bloc in li_blocs]
		return li_lines
	...
	def lines_in_bloc(text_bloc):
		...
		li_len_words[0]+=DSConstants['NB_CHAR_PER_INDENT'] # add the indent length to the first word
		
		counter = 0		
		lines =[]
		line =[]
		for i,v in enumerate(li_words):
			if counter+len(v)> DSConstants['NB_CHAR_PER_LINE']: 
				# if it ovetakes the line then
				if counter==0: #if the word is bigger than the line
					lines_to_add = []
					for i in range(len(v)/(DSConstants['NB_CHAR_PER_LINE']-1)+1):
						# We count -1 for the size of the breakline
						ii = i*(DSConstants['NB_CHAR_PER_LINE']-1) 
						jj = min(len(v),
									(i+1)*(DSConstants['NB_CHAR_PER_LINE']-1))
						w = v[ii:jj]
						if jj<len(v):
							w+='-'
	...
	def stats(text,format=['nb_pages']):
		...
		lines = DSDocStatistics.lines_in_book(text)
		results=[]
		for ask in format:
			if ask=='nb_pages':
				lines_flat = [item for bloc in lines for item in bloc]
				q = len(lines_flat)/DSConstants['NB_LINE_PER_PAGE']
				r = len(lines_flat)%DSConstants['NB_LINE_PER_PAGE']
				if r>0:
					q+=1
				results.append(q)
			elif ask =='nb_blocs':
				...
				book = DSDocStatistics.shape_book(text)
	...
	def shape_book(text):
		...
		result=u""
		for block in lines:
			result+=' '*DSConstants['NB_CHAR_PER_INDENT']
			for line in block:
				result+=u' '.join(line)+u'\n'
		return result
		
class DSDialogManager(QtGui.QDialog):
	...
	def refresh(self,text=False):
		...
		if text:
			res = DSDocStatistics.stats(text,self.info)
		else :
			res = DSDocStatistics.stats(unicode(self.textedit.toPlainText()),self.info)
