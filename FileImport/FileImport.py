from FileImportConstants import FIConstants
...
class FIImportGeneral:
	...
	def save_file(self,filepath=None,outdir=None):
		...
		- filepath : the complete path where to save the file (with extension)
			...
			to save the file.
			Note : if the FIImport instance was not created with a 'file' argument, 
...
class FIImportHtml (FIImportGeneral):
	extension='html'
	def __init__(self,file=None,fromtext=None):
		"""
		A general class for importation (usually used with re-implementations)
...
class FIImportOdt (FIImportGeneral):
	extension='odt'
	def __init__(self,file=None,fromtext=None):
		"""
		A general class for importation (usually used with re-implementations)
	...
	def import2xml(self):
		...
		import_html = FIImportHtml(fromtext=self.raw_text)
	...
	def odt2html(self,skip=False):
		...
		if not skip or not os.path.exists(newfile):
			to_call = FIConstants['LIBREOFFICE_COMMAND'] + ' ' + \
						args + ' ' +\
...
class FIImportDoc (FIImportOdt):
	extension='doc'
	
		
class FIImportDocx (FIImportOdt):
	extension='docx'
	
class FIImportTxt (FIImportGeneral):
	extension='txt'
...
FIList = [FIImportOdt,FIImportHtml,FIImportDoc,FIImportDocx,FIImportTxt]
