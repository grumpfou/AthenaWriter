...
class TEWordSet (set):
	...
	WordSet.addWord("apple",TEWordTools.IND_LOWER|TEWordTools.IND_FIRST_CAP) 
	...
	def add(self,word,id=None):
		if id==None:
			...
			if id == TEWordTools.IND_LOWER:
				id=TEWordTools.IND_ALL
	...
	def add(self,word_entry,id=None): # if id is None, all the version will be possible
		if id==None:
			id = TEWordTools.whatID(word_entry)
			if id == TEWordTools.IND_LOWER:
				id=TEWordTools.IND_ALL
...
class TEWordDico (dict):
	...
	def __getitem__(self,word):
		...
		id=TEWordTools.whatID(word)
		if id|res[1]>0:
			return TEWordTools.toID(res[0],id)
		else: 
			raise KeyError(word)
...
if __name__=='__main__':
	d = TEWordDico({'Toto':'totoro','titi':'Titiri'})
